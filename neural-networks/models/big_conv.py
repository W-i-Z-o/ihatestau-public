...
def cnn_model_fn(features, labels, mode):
    ...
    # Dense Layer
    pool2_flat_a = tf.reshape(pool2_a, [-1, IMAGE_H//16 * IMAGE_W//16 * 64])
    pool2_flat_b = tf.reshape(pool2_b, [-1, IMAGE_H//16 * IMAGE_W//16 * 64])
    dense = tf.layers.dense(inputs=tf.concat([pool2_flat_a, pool2_flat_b], 1), units=1024, activation=tf.nn.relu)
