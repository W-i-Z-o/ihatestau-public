...
def cnn_model_fn(features, labels, mode):
    ...
    jpeg_input = tf.map_fn(jpeg_decode, features['image-a'], dtype=tf.float32, back_prop=False, parallel_iterations=10)
...
def train(training_images, model_dir, steps):
    ...
    train_filelist  = [a for a,b,m,l in training_images]
...
def run(images, model_dir):
    ...
    filelist = np.asarray([a for a,b,m in images], dtype=np.str)
